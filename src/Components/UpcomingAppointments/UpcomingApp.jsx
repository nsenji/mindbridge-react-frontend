...
export default function UpcomingApp() {
  const { isLoading1, error, data } = useQuery("getToken2", isValidToken, {
    enabled: true,
  });
      ...
      const getAppointments = async () => {
        try {
          ...
          let scheduledAppointments =
            await Appointments.getScheduledAppointments({
      ...
      <div className="todaytab">
        {appointmentsToday.length ? (
          appointmentsToday.map((app) => (
            <Appointment
              key={app.selected_apt_ID}
              id={app.id}
              patientName={app.patient.name}
              date={app.date}
              time={app.time}
              status={app.status}
              diagnosis={app.patient.diagnoses[0].result}
              patientEmail={app.patient.email}
            />
          ))
        ) : (
          <h5 className="noactivity">
            No Activity <FaRegCalendarTimes />
          </h5>
        )}
      </div>
