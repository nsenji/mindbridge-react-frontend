...
const ScheduleRoute = () => {
    ...
    const { isLoading1, error, data, refetch } = useQuery("getToken3", isValidToken, { enabled: true })
            ...
            <div className='flex flex-col flex-grow  my-2 mr-2'>
                <div className='h-[8%]  flex items-center justify-between'>
                    ...
                    <div className='flex items-center mr-5'>
                        <CustomButton content={"Create schedule"} classname={"bg-dark-blue text-white w-[150px]"} handleButtonClick={handleClickOpen} />
                        <CustomizedDialogs alertState={open} onClose={handleClose} handleSubmit={handleSubmit} userData={userData}/>
...
export function CustomizedDialogs({ alertState, onClose, handleSubmit, userData }) {

    const [datetime, setDateTime] = useState({ date: null, time: null, doctorID: userData.doc_ID, status: "active" })
        ...
        <React.Fragment>
            ...
            >
                ...
                <DialogContent className='flex flex-col items-center justify-evenly w-[600px] mb-6' >
                    <p className='text-lg font-semibold text-white'>Select date and time</p>
                    <form className=' flex flex-col h-[200px] justify-between' onSubmit={(e) => handleSubmit(e, datetime)}>
                        <input type='date' className= 'w-[200px] px-3 py-3 border bg-[#353c5e] border-white text-white rounded focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500' onChange={(e) => setDateTime({ ...datetime, date: e.target.value })} />
                        <input type='time' className='w-[200px] px-3 py-3 border bg-[#353c5e] border-white text-white rounded focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500' onChange={(e) => setDateTime({ ...datetime, time: e.target.value })} />
                        <button type='submit' className='h-12 font-bold px-6 border border-white text-lg hover:bg-dark-blue text-white cursor-pointer rounded-lg w-[200px]'>Save</button>
                    </form>
